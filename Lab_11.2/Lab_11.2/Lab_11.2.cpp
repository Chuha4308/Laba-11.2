...
string processLine(const string& line) {
    ...
    while (ss >> token) {
        size_t dotPos = token.find('.'); // Знайти позицію крапки

        if (dotPos != string::npos && dotPos + 1 < token.length() && isdigit(token[dotPos + 1])) { // Перевірити, чи є цифри після крапки
            size_t digitCount = 0;
            for (size_t i = dotPos + 1; i < token.length(); ++i) {
                if (isdigit(token[i])) {
                    digitCount++;
                    if (digitCount > 2) { // Якщо цифр більше двох після крапки
                        token.erase(i); // Видалити зайві цифри
                        break;
                    }
                }
                else {
